...
TEST_F(GameTestCase, AddBotPlayer) { EXPECT_EQ(p1, add_player(1, true)); }

TEST_F(GameTestCase, InstallShips) { EXPECT_NO_THROW(install_ships(p)); }

TEST_F(GameTestCase, MakeShot) { EXPECT_NO_THROW(make_shot(p, 0)); }
